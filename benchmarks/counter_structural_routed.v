...
DFF _23_ (.C(clk_buf_1_out), .D(_01_), .Q(cnt[1]));
AND2 _12_ (.A(cnt[0]), .B(cnt[1]), .Y(_04_));
DFF _24_ (.C(clk_buf_1_out), .D(_03_), .Q(cnt[2]));
NOR2 _13_ (.A(cnt[0]), .Y(_05_));
AND2 _21_ (.A(_07_), .B(_10_), .Y(_03_));
DFF _22_ (.C(clk_buf_2_out), .D(_00_), .Q(cnt[0]));
...
DFF _25_ (.C(clk_buf_2_out), .D(_02_), .Q(cnt[3]));
