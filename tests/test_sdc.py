...
def run_suite():
    ...
    try:
        ...
        check("load_design (stage 1)", False, str(e))
        return summarize()
    ...
    try:
        ...
        check("STA WNS is finite after SDC load", False, str(e))
        check("WNS >= 0 with 2 ns clock (500 MHz)", False, "stage failed")
        wns_sdc = 0.0
    ...
    try:
        ...
        check("WNS < 0 with 10 ps clock (violation expected)", False, str(e))
    ...
    try:
        ...
        check("SDC output/input delays reduce WNS vs unconstrained", False, str(e))
    ...
    try:
        ...
        check("set_false_path / multicycle SDC parsed without error", False, str(e))
        check("clock period 1 ns = 1000 ps", False, "stage failed")
    finally:
        if os.path.exists(fp_sdc): os.remove(fp_sdc)
    ...
    try:
        ...
        check("STA without SDC still produces finite WNS", False, str(e))
    ...
    try:
        ...
        check("violations > 0 with 1 ps clock (baseline)", False, str(e))
        check("false_path on u_buf0 reduces violation count", False, "stage failed")
    finally:
        if os.path.exists(fp7_sdc): os.remove(fp7_sdc)
    ...
    try:
        ...
        expected_delta = UNCERTAINTY_NS * 1000
        ...
        check("clock_uncertainty shifts WNS by correct amount (100 ps)", False, str(e))
        wns_no_unc = 0.0
    finally:
        if os.path.exists(unc8_sdc): os.remove(unc8_sdc)
    ...
    try:
        ...
        expected_lat_delta = LATENCY_NS * 1000
        ...
        check("clock_latency increases WNS by correct amount (50 ps)", False, str(e))
    finally:
        if os.path.exists(lat9_sdc): os.remove(lat9_sdc)
    ...
    try:
        ...
        check("multicycle SDC: clock period 100 ps parsed", False, str(e))
        wns_mc = -1.0
    finally:
        if os.path.exists(mc10_sdc): os.remove(mc10_sdc)
